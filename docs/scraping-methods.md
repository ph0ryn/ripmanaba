...
すpathはoriginを除いた `ManabaPath` とする。
...
コース名セルのコースリンクから `path` と `id` を取得する。実DOMでは同じ
...
pathになる。JSONの `path` には詳細pathを使う。
...
存在すれば `upload.enabled: true` とみなす。最大ファイルサイズは同じform内
の `※１ファイルにつき ...` の文から `maxFileSizeLabel` として保持する。
...
あれば `ManabaPath`、外部URLなら文字列として保持する。
...
このpathを `AttachmentInfo.path` に保持し、リンクテキスト末尾の日時を
`uploadedAt` にする。
同じ添付ファイルに対して、アイコン用の空テキストリンクとファイル名表示リ
ンクが重複して出る場合がある。`path` で重複排除し、日時はテキストがあるリ
ンクから取る。
...
| タイトル | `title`, `path`, `id` |
| コース | `course.name`, `course.path`, `course.id` |

タイトルリンクがない `(非公開の課題)` の行は `path` を省略し、kindは
`unknown` にする。
